...
$(document).ready(function(e) {
  ...
  $('button').click(function(e) {
    ...
    $('button').removeClass();
    $this.addClass($play);
    $('.result').empty().html('<div class="animated fadeOutUp ' + $play + '">' +
						  message + '</div>');
  });
  ...
  function play(selection) {
    ...
    if (selection === cpuPlays) {
      message = cpuPlays + ' tied with ' + selection;
      moveCount++;
      tieCount++;
	...
	function recordScore($type, $player, $cpu) {
		let comparator = $type === 'win' ? '>' : $type === 'loss' ? '<' : '=';
		$('.history').prepend('<div class="history-item ' +
					 $type + '">' +
					 '<i class="fa fa-hand-' + $player + '-o"></i>'
					 + '<span class="small">' + comparator + '</span>' +
					 '<i class="fa fa-hand-' + $cpu + '-o"></i>'
					 + '</div>');
